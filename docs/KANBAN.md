...
| **Step 3: Move the “Brains” (Data & Logic)** | 🔄 **(bijna klaar)** |  |
| ↳ Relocate data processing logic to ``src/data/data_loader.py`` | ✔️ |  |
| ↳ Move feature engineering logic to ``src/features/engineering.py`` | ✔️ |  |
| ↳ Add TA‑based prediction engine (``src/features/prediction.py``) | ✔️ |  |
| ↳ Add ML pipeline skeleton (``src/ml/…``) | ✔️ |  |
